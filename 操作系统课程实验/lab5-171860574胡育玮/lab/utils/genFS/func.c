...
int calGroupNum (int sectorNum, int sectorsPerBlock) { //calculate the num of groups
...
// sector as unit: how many sectors in a group
{ 
...
int mkdir (const char *driver, const char *destDirPath) {
	...
	tmp = *((char*)destDirPath + size + 1);  //1st byte after last '/'
	*((char *)destDirPath + size + 1) = 0;   //set 1st byte after last '/' to \0
	ret = readInode(file, &superBlock, groupDesc,
		&fatherInode, &fatherInodeOffset, destDirPath);  //open inode of father dir
	*((char*)destDirPath + size + 1) = tmp;  //resume
	if (ret == -1) {
