...
//eth0: 00:0c:29:3c:e1:47
//eth1: 00:0c:29:3c:e1:51
char IPPC1[INET_ADDRSTRLEN] = "192.168.3.3"; //IP:与PC2相连的网卡
char IPR2[INET_ADDRSTRLEN] = "192.168.2.2";  //IP:与router1相连的网卡
char NextIP[INET_ADDRSTRLEN] = "192.168.2.1"; //下一跳IP：router2的eth1网卡
char PC2IP[INET_ADDRSTRLEN] = "192.168.3.2";  //下一跳IP：PC2的网卡
...
char IFNAME1[] = "eth0";   //与PC2相连的网卡
char IFNAME2[] = "eth1";   //与router1相连的网卡
//uint8_t destMacPC1[6] = {0x00,0x0c,0x29,0x24,0x28,0x2c}; //PC2的网卡
//uint8_t destMacR2[6] = {0x00,0x0c,0x29,0xaa,0xc7,0xb0};  //router1与router2相连的网卡: router1 eth1
uint8_t destMacPC1[6]; //PC2的网卡
uint8_t destMacR2[6];
...
void init_router() //初始化路由表，设备信息表：MAC地址和IP地址，设备信息等
{
	...
	if (ioctl(sfd, SIOCGIFINDEX, &req) < 0) //一定要建立socket之后才能用socket号来获取那些接口index信息
	{
		perror("无法获得Router1网卡ifindex！\n");
		return;
	}
	device[1].ifIndex = req.ifr_ifindex; //直接获得ifindex
	printf("连接Router1的网卡的ifIndex = %u\n", device[1].ifIndex);

	//  get mac addr：获取接口的MAC地址
	if (ioctl(sfd, SIOCGIFHWADDR, &req) < 0)
	{
		perror("无法获得Router1网卡MAC地址!\n");
		return;
	}
	memcpy(device[1].mac_addr, &(req.ifr_hwaddr.sa_data), ETH_ALEN);
	printf("连接Router1的网卡的MAC地址 = %02X:%02X:%02X:%02X:%02X:%02X\n", HWADDR(device[1].mac_addr));

	close(sfd);

	//=======================================below: set route table==========================================

	route_table[0].range = (uint32_t)inet_addr(net2);   //通往router1
	...
	route_table[1].range = (uint32_t)inet_addr(net1);   //通往PC1
	...
	route_table[2].range = (uint32_t)inet_addr(net3);   //通往PC2
	strcpy(route_table[2].interface, IFNAME1);
	memcpy(route_table[2].dest_MAC, destMacPC1, 6);
	route_table[2].nexthopip = (uint32_t)inet_addr(PC2IP);
...
void routing()
{
	...
	case ETH_P_ARP:
		...
		if (arp->arp_tpa == device[1].IP && ntohs(arp->arp_op) == ARPOP_REQUEST) //来自router1的ARP请求
		{
			printf("收到了来自Router1的ARP包\n"); 
			//Route table 0 1
			memcpy(route_table[0].dest_MAC, arp->arp_sha, 6); //通往子网2
			memcpy(route_table[1].dest_MAC, arp->arp_sha, 6); //通往子网1

			//route_table[2].nexthopip = arp->arp_spa;
			//uint32_t ttemp = arp->arp_spa; //Router1的IP地址

			arp->arp_op = htons(ARPOP_REPLY);				  //reply
			memcpy(arp->arp_sha, device[1].mac_addr, 6);	  //设置源MAC
			arp->arp_spa = device[1].IP;					  //设置源IP
			memcpy(arp->arp_tha, route_table[0].dest_MAC, 6); //设置目的MAC
			arp->arp_tpa = route_table[0].nexthopip;		  //设置目的IP

			memcpy(eth->h_source, device[1].mac_addr, 6);
			...
			{
				perror("To Router2 ARP Reply Sendto error!\n");
				exit(-1);
			}

			else
				printf("回复Router2的ARP包发送成功！\n");
		}
	...
	case ETH_P_IP:
		...
		if (cnt == 6) //说明这个destmac空缺
		{
			...
			{
				perror("To PC2 ARP sendto error！\n");
				exit(-1);
			}
			else
				printf("To PC2 ARP请求发送成功!\n");
			...
			while (1)
			{
				...
				{
					printf("Received wanted PC2 ARP reply!\n");
